...
Renderer::Renderer(Window &parent) : OGLRenderer(parent) {
  ...
  glFramebufferTexture2D(
    GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, bufferColourTex, 0
  );
  glFramebufferTexture2D(
    GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, bufferNormalTex, 0
  );
  glFramebufferTexture2D(
...
void Renderer::DrawPointLights() {
  ...
  Vector3 translate = Vector3(
    RAW_HEIGHT * HEIGHTMAP_X / 2.0f, 500, RAW_HEIGHT * HEIGHTMAP_X / 2.0f
  );
...
void Renderer::CombineBuffers() {
  ...
  glUniform1i(
    glGetUniformLocation(currentShader->GetProgram(), "emissiveTex"), 3
  );
  glUniform1i(
    glGetUniformLocation(currentShader->GetProgram(), "specularTex"), 4
  );
